...
def setFileName(pathPrefix = ''):
    ...
    if not os.path.isdir(os.path.join(pathPrefix, '%s'%year, '%02d'%month, '%02d'%day)):
        ...
        os.chmod(os.path.join(pathPrefix, '%s'%year, '%02d'%month, '%02d'%day), 0775 | stat.S_ISGID)
        os.chown(os.path.join(pathPrefix, '%s'%year, '%02d'%month, '%02d'%day), fileUid, fileGid)

    return os.path.join(pathPrefix, '%s'%year, '%02d'%month, '%02d'%day, 'tilosradio-%02d%02d%02d-%02d%02d.mp3'%(year, month, day, hour, min))
...
def createDaemon():
    ...
    os.chown(fileName, fileUid, fileGid)
    print 'parent quits'
    sys.exit(0)
